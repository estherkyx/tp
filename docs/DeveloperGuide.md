...
The diagram below shows the class structure of the `Model` component and how `ModelManager` coordinates the various data structures and interfaces.

<img src="images/CoreModelStructureClassDiagram.png" width="550" />

**Domain Entities**

The diagram below illustrates the internal structure and relationships of the core domain entities (Person and TuitionClass) managed by the Model component:
<img src="images/DomainEntitiesClassDiagram.png" width="550" />

The `Model` component,

* stores the address book data i.e., all `Person` and `TuitionClass` objects (which are contained in a `UniquePersonList` object and `UniqueClassList` object respectively).
* stores the currently 'selected' `Person` objects (e.g., results of a search query) as a separate filtered list `filteredPersons` which is exposed to outsiders as an unmodifiable `ObservableList<Person>` that can be 'observed' e.g. the UI can be bound to this list so that the UI automatically updates when the data in the list change.
* the filtered tuition class list `filteredTuitionClasses` is architecturally in place to support a future Ul panel for tuition classes. The mechanism exists within `Model` to support filtering and display.
...
&emsp;1a. The user enters an invalid command.<br>
&emsp;&emsp;&emsp;1a1. TutorFlow shows an error message: _"Unknown command"._

&emsp;&emsp;&emsp;Use case ends.

&emsp;1b. The user enters a command with an invalid format.<br>
&emsp;&emsp;&emsp;1b1. TutorFlow shows an error message: _"Invalid command format!"_
...
&emsp;1d. The user provides an empty name.<br>
&emsp;&emsp;&emsp;1d1. TutorFlow shows an error message: _"Names should only contain alphanumeric characters<br>
&emsp;&emsp;&emsp;&emsp;&emsp;and spaces, and it should not be blank"._

&emsp;&emsp;&emsp;Use case ends.

&emsp;1e. The user provides an invalid phone number (e.g., not at least 3 digits/contains non-numbers).<br>
&emsp;&emsp;&emsp;1e1. TutorFlow shows an error message: _"Phone numbers should be Singaporean numbers:<br>
&emsp;&emsp;&emsp;&emsp;&emsp;exactly 8 digits starting with 3, 6, 8, or 9"._

&emsp;&emsp;&emsp;Use case ends.

&emsp;1f. The user provides an invalid email format.<br>
&emsp;&emsp;&emsp;1f1. TutorFlow shows an error message: _"Emails should be of the format local-part@domain"._

&emsp;&emsp;&emsp;Use case ends.

&emsp;1g. The provided name and category already exists in the contact list (person already exists).<br>
&emsp;&emsp;&emsp;1g1. TutorFlow shows an error message: _"This person already exists in the address book"._
...
&emsp;1a. The user provides an invalid day format.<br>
&emsp;&emsp;&emsp;1a1. TutorFlow shows an error message: _"Day must be a valid day of the week (e.g., MONDAY,<br>
&emsp;&emsp;&emsp;&emsp;&emsp;Tuesday)."_
...
&emsp;1c. The user provides a day and timing that conflicts with an existing class.<br>
...
&emsp;2a. There are no classes to display.<br>
&emsp;&emsp;&emsp;2a1. TutorFlow shows an empty list and displays the message: _"There are no tuition classes<br>
...
&emsp;1a. The name given is not a tutor.<br>
&emsp;&emsp;&emsp;1a1. TutorFlow shows an error message: _"Tutor with name '[TUTOR_NAME]' not found."_
...
&emsp;1a. Either the student or parent name is not found.<br>
&emsp;&emsp;&emsp;1a1. TutorFlow shows an error message: _"The person with name [STUDENT_NAME/<br>
&emsp;&emsp;&emsp;&emsp;&emsp;PARENT_NAME] could not be found."_

&emsp;&emsp;&emsp;Use case ends.

&emsp;1b. The specified parent is not categorised as `parent`.<br>
&emsp;&emsp;&emsp;1b1. TutorFlow shows an error message: _"The person [PERSON_NAME] is not a Parent."_

&emsp;&emsp;&emsp;Use case ends.

&emsp;1c. The specified student is not categorised as `student`.<br>
&emsp;&emsp;&emsp;1c1. TutorFlow shows an error message: _"The person [PERSON_NAME] is not a Student."_
...
&emsp;1a. The student name is not found.<br>
&emsp;&emsp;&emsp;1a1. TutorFlow shows an error message: _"The person with the name '[PERSON_NAME]'<br>
&emsp;&emsp;&emsp;&emsp;&emsp;could not be found."_

&emsp;&emsp;&emsp;Use case ends.

&emsp;1b. The class timeslot is invalid or does not exist.<br>
&emsp;&emsp;&emsp;1b1. TutorFlow shows an error message: _"The class at the specified timeslot does not exist."_
...
&emsp;1c. The student is not linked to the class.<br>
&emsp;&emsp;&emsp;1c1. TutorFlow shows an error message: _"The student is not linked to this class"._
...
&emsp;1a. The class day or time is invalid.<br>
&emsp;&emsp;&emsp;1a1. TutorFlow shows an error message: _"There is no class at the specified day and time"._

&emsp;&emsp;&emsp;Use case ends.

&emsp;2a. The class has no enrolled students or no assigned tutor.<br>
&emsp;&emsp;&emsp;2a1. TutorFlow displays placeholders indicating missing information (e.g., _"Tutor: None"_).
...
2.  TutorFlow displays a filtered list containing only contacts categorised as `student`.
...
&emsp;3a. The given index is not an integer.<br>
...
&emsp;&emsp;&emsp;3b1. TutorFlow shows an error message: _"The person index provided is invalid. Please<br>
&emsp;&emsp;&emsp;&emsp;&emsp;provide a valid index in the displayed list."_

&emsp;&emsp;&emsp;Use case ends.

&emsp;3c. No fields are provided to edit.<br>
&emsp;&emsp;&emsp;3c1. TutorFlow shows an error message: _"At least one field to edit must be provided."_

&emsp;&emsp;&emsp;Use case ends.

&emsp;3d. The user provides an invalid name.<br>
&emsp;&emsp;&emsp;3d1. TutorFlow shows an error message: _"Names should only contain alphanumeric characters<br>
&emsp;&emsp;&emsp;&emsp;&emsp;and spaces, and it should not be blank"._
...
&emsp;3f. The user provides an invalid email format.<br>
&emsp;&emsp;&emsp;3f1. TutorFlow shows an error message: _"Emails should be of the format local-part@domain"._

&emsp;&emsp;&emsp;Use case ends.

&emsp;3g. The user provides an invalid category.<br>
&emsp;&emsp;&emsp;3g1. TutorFlow shows an error message: _"Invalid category"._

&emsp;&emsp;&emsp;Use case ends.

&emsp;3h. The edited details would duplicate an existing person (same identifying fields).<br>
&emsp;&emsp;&emsp;3h1. TutorFlow shows an error message: _"This person already exists in the address book"._
...
2. **Expand `Phone` field validation to accept international codes and formatting:** The current phone number validation is overly strict, permitting only a limited Singapore 8-digit numerical format. This flaw prevents users from saving valid numbers that include international country codes (e.g. `+65`, `+1`) or common formatting characters like spaces and hyphens (e.g. `8145-7829`), which are common in real-world use. We plan to modify the validation logic for the `Phone` field to permit an optional leading `+` symbol, digits, spaces, and hyphens. This change will allow users to successfully store a wider variety of realistic phone numbers, such as `+1 234 5678` or `9876-5432`, instead of having them rejected.

3. **Allow students to be linked to multiple parents:** The current architecture only permits a student to be associated with one parent at a time. This causes the `linkParent` command to overwrite any existing parent-student association. For example, if "Child 1" is linked to "Parent 1", executing `linkParent n/Child 1 n/Parent 2` disconnects "Child 1" from "Parent 1". This does not reflect real-world scenarios where a student may have two contactable parents. We plan to refactor the data model to support a many-to-many relationship between students and parents. The `linkParent` command will be modified to add a new association to the student's record, rather than replacing the current one. This will allow "Child 1" to be successfully linked to both "Parent 1" and "Parent 2" simultaneously.

4. **Standardise person referencing to allow `INDEX` in all relevant commands:** The application currently has an inconsistent referencing system. Commands like edit use a person's index (e.g. `edit 1 ...`), while other commands like linkClass require the person’s full name (e.g. `linkClass d/DAY ti/TIME n/NAME`). This is jarring for users, who must switch from referencing a person by their index to referencing them by their name. We plan to update all commands that currently require a name (e.g. `linkClass`, `unlinkClass`, `linkParent`, `getParent`, `getStudents`, `getClasses`) to also accept an `INDEX` parameter. For example, instead of a user having to type `linkClass d/monday ti/H12 n/John`, they will be able to use the person's index directly, such as `linkClass 1 d/monday ti/H12`, making the workflow smoother and more consistent.

5. **Allow partial name matching for contact-referencing commands:** The application currently requires users to type the exact, full name for commands that reference a contact (e.g. `getParent n/Johnathan Doe`). This is inconvenient and error-prone, as users must remember the exact name, and any typo or omission (e.g. `getParent n/John`) will cause the command to fail. We plan to update the command parser to accept partial, case-insensitive name matches. If the provided partial name uniquely identifies one person, the command will execute for that person. For example, if "Johnathan Doe" is the only contact whose name contains "John", the command `getParent n/John` will succeed. If the partial name is ambiguous (e.g., "John" matches "Johnathan Doe" and "John Tan"), the application will report an error listing the ambiguous matches.

6. **Enable day-of-week abbreviations for command inputs:** The current command parser requires users to type the full name for days of the week (e.g. `monday`). This is inefficient and verbose for our target users (fast typists) who are accustomed to using standard abbreviations. We plan to enhance the day parser to accept common abbreviations in addition to the full names. For example, inputs like `mon`, `tue`, `wed`, `thu`, `fri`, `sat`, and `sun` will be correctly recognized and mapped to their corresponding full day names (`monday`, `tuesday`, etc.), making command entry faster and more user-friendly.

7. **Add functionality to edit existing class timings:** The application currently lacks a command to modify a class's day or time after its creation. This is a significant problem as class schedules frequently change. The only workaround is to `unlinkClass` every person, `createClass` with the new timing, and then `linkClass` every person to the new class, which is extremely inefficient and error-prone. We plan to introduce an `editClass` command. This command will allow users to directly modify an existing class's attributes (like day or time) while preserving all associated student/tutor links. For example, `editClass d/Monday ti/H12 nd/Tuesday nti/H14` would update the class’s tutor and students class timing from  "Monday 1200" to "Tuesday 1400" without requiring any relinking.

8. **Add functionality to delete existing classes:** The application currently provides no mechanism to remove a class once it has been created. Users who make a mistake during creation or have classes that are no longer running are forced to keep them in the system, leading to a cluttered and confusing class list. We plan to introduce a `deleteClass` command. This command will allow a user to permanently remove a class by specifying its day and time (e.g. `deleteClass d/monday ti/H12`). When a class is deleted, all existing links between that class and any students or tutors will be automatically removed.

9. **Change class timing from fixed slots to a flexible 24-hour format:** The application currently restricts the `TIME` input to a rigid, predefined list of 2-hour slots between 12pm and 8pm (H12, H14, H16, H18, H20). This is a significant problem that makes the application unusable for any tuition center with classes at other realistic times (e.g., 10:00 AM, 1:30 PM) or with durations other than two hours. We plan to refactor this feature to accept a specific time in a 24-hour format. This will allow users to create classes with precise start times, such as `createClass ... ti/1000` or `createClass ... ti/1330`, instead of being limited to the set of pre-fixed timeslots.

10. **Expand support for concurrent classes and multiple subjects:** The application was initially designed under the core assumption of targeting a small, single-classroom GP tuition centre. This led to an intentional constraint preventing the creation of more than one class at the exact same day and time, reflecting the physical limitation of a single-room operation. We plan to expand the application's scope to support larger tuition centres that run parallel sessions. To achieve this, we will refactor the `Class` entity by adding a mandatory subject field. The uniqueness check will be changed from `(day, time)` to `(subject, day, time)`. This will allow users to create concurrent classes, for example: `createClass s/GP d/monday ti/H12` and `createClass s/Math d/monday ti/H12`. This structural change will also enable refactoring the `Student` model to allow a student to be linked to multiple, distinct classes, which is common in larger tuition centres. 
